...
# 首次部署创建管理员（已存在则保留原密码，不重置）
if node scripts/ensure-admin-user.mjs; then
  echo "✓ 管理员账号已就绪（重置密码: node scripts/create-admin-user.mjs <用户名> <新密码>）"
else
  echo "⚠️  管理员账号初始化失败，请检查 .env 中的 ADMIN_USERNAME / ADMIN_PASSWORD"
fi
