#!/usr/bin/env python3
"""从本机浏览器导出 x.com Cookie 到 cookies/x.txt"""
...
OUT = os.path.join(ROOT, "cookies", "x.txt")
DOMAINS = (".x.com", "x.com", ".twitter.com", "twitter.com", "mobile.twitter.com")
SITE_KEY = "twitter"
...
def preferred_browsers():
    env_browser = (
        os.environ.get("TWITTER_BROWSER")
        or os.environ.get("SOCIAL_COOKIES_FROM_BROWSER")
...
def export_from_browser(browser_cookie3, browser_name):
    ...
    if count == 0:
        try:
            for c in get_fn():
                if c.domain and (SITE_KEY in c.domain or c.domain.endswith(".x.com")):
                    add_cookie(c)
...
def export_via_ytdlp(browser_name):
    ...
    ytdlp = os.path.join(ROOT, ".local", "bin", "yt-dlp")
