...
| [影视搜索](/tools/media-search) | 汁源导航，多个搜索入口备用，也可进本站网盘检索 | 打开页面 → 点蓝色链接进入搜索站 |
| [影视资源下载](/tools/media-download) | 按片名从多厅找网盘资源，找到就能复制分享 | 输入片名 → 浏览搜索结果 → 展开条目并复制需要的链接 |
...
**AI 对话**默认走 DeepSeek，轻松闲聊为主；你说清楚想做什么时，也会帮你跳转工具、预填链接。首页对话会根据问题自动在 DeepSeek 与火山方舟之间切换（例如附图识别走火山方舟）；全屏对话页仍可手动切换模型。若需固定单一提供商，可在 `.env` 设置 `CHAT_PROVIDER=deepseek` 或 `ark`。
